 ...
 */
function onFinish (message, listener) {
...
function attachListener (message, listener) {
  ...
  if (!attached || !attached.queue) {
    attached = message.__onFinished = createListener(message)
...
function attachFinishedListener (message, attached) {
  ...
  const eeThunk = thunk = first([[message, 'end', 'finish']], onFinish)
  if (message.socket) {
...
onFinish.isFinished = isFinished
